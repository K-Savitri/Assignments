...
float updatew1(float w1, float doh1)
{
       float ld =0.1;
    return w1 -(ld*(doh1));
}
float updatew2(float w2, float doh2)
{
      float ld =0.1;
    return w2 -(ld*(doh2));
}
int main()
{
    ...
    for(int i=0; i<epoch;i++){
        cout<<"This is epoch no "<<i+1<<endl;
    for (int i = 0; i < 4; i++) {
        int k=0;
    ...
    float grad1 = gradientw1( x1,  x2,  y,  w1,  w2);
     float grad2 = gradientw2( x1,  x2,  y,  w1,  w2);
 
    
        w1 = updatew1(w1,grad1);
        w2 = updatew2(w2, grad2);

      
       cout<<" w1 is "<< w1<<" ---- w2 is "<<w2<<endl;
    }
}      
}
